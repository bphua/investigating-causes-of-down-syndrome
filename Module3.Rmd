...
We explore the data graphically and perform a quick numerical analysis using one-way ANOVA informally.

```{r}
ggplot(data, aes(y=weight, x=tg)) + 
  geom_boxplot() + 
  theme_classic()
...
When we do look at them separately, we can see that there is no significant difference (p-value = 0.515 > 0.05) within the male and female weights of transgenic and nontransgenic mice.
...
$H_0: $ One of $\mu_0, \mu_1, \mu_2, \mu_3, \mu_4$ does not equal to the other
...
(Not sure if we're looking at interaction here...)
